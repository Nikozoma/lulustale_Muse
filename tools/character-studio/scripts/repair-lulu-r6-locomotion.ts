...
const BASELINE_ROOT = path.join(
  ...
  "lulu",
  "history",
  "revision-5-locomotion-baseline"
);
const IMPORT_ROOT = path.join(
  ...
  "lulu",
  "imports",
  "canonical-anime-r6-locomotion-engineering-repair"
);
...
const PROFILE_ROWS = [
  {
    ...
    row: 2,
    input: "left-walk-r6-authored-source.png",
    reference: "left-walk-r5-reference.png"
  },
  {
    direction: "Right" as const,
    row: 6,
    input: "right-walk-r6-authored-source.png",
    reference: "right-walk-r5-reference.png"
  }
];
...
async function main(): Promise<void> {
  ...
  for (const profile of PROFILE_ROWS) {
    await writeProfileReference(
      walkRaw.data,
      walkRaw.info.width,
      profile.row,
      path.join(IMPORT_ROOT, profile.reference)
    );
  }
  ...
  for (const profile of PROFILE_ROWS) {
    ...
    profileReports.push({
      ...
      baselineReference: projectPath(path.join(IMPORT_ROOT, profile.reference)),
  ...
  const report = {
    ...
    diagnosis: {
      fringe:
        "Neutral antialias colors remained exposed around the idle/walk exterior.",
      profileWalk:
        "Revision-5 Left/Right rows mixed oversized contact poses, neutral poses, and repeated high-knee poses, so the cycle lacked stable temporal gait phases.",
      verticalCadence:
        "At default full-strength running, 240px/s over a 160px cycle produced 1.5 cycles/s and 12 frame changes/s, making Up/Down read overcranked.",
      roots:
        "Authored roots were already stable; no root or world movement-speed correction was required.",
      diagonals:
        "Diagonal silhouettes and phase order were coherent enough to preserve."
    },
    scope: {
      changedActions: ["idle", "walk", "run"],
      profileRowsReauthored: ["Left", "Right"],
      verticalRowsPreserved: ["Down", "Up"],
      diagonalRowsPreserved: ["Down-Left", "Up-Left", "Up-Right", "Down-Right"],
      runPixelsPreserved: true,
      runPlaybackAdjusted: true
    },
    contract: {
      cell: [CELL, CELL],
      root: [ROOT_X, ROOT_Y],
  ...
  console.log(`run-pixels-preserved: ${report.outputs.run.sha256}`);
}

async function writeProfileReference(
  sheet: Buffer,
  sheetWidth: number,
  sourceRow: number,
  outputPath: string
): Promise<void> {
  const referenceWidth = CELL * 4;
  const reference = Buffer.alloc(referenceWidth * CELL * 2 * 4);
  for (let frame = 0; frame < 8; frame += 1) {
    writeCell(
      ...
      Math.floor(frame / 4),
      frame % 4,
      readCell(sheet, sheetWidth, sourceRow, frame)
    );
  }
  await writeFile(
    outputPath,
    await encode(reference, referenceWidth, CELL * 2)
  );
}
...
async function packProfileFrames(
  ...
  const phaseOrder =
    direction === "Left"
      ? [0, 2, 1, 3, 4, 6, 5, 7]
      : [0, 1, 2, 3, 4, 6, 5, 7];
  const ordered = phaseOrder.map((sourceIndex) => sourceOrder[sourceIndex]);
...
function cleanBrightCell(input: Buffer): { data: Buffer; recolored: number } {
  ...
  for (let y = 0; y < CELL; y += 1) {
    for (let x = 0; x < CELL; x += 1) {
      ...
      if (average < 48 || chroma > 56) continue;
